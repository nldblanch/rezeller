  ...
  return (
    ...
    >
      <Link href={`/items?category=${category}`}>
        <li className="w-full h-full p-2">{convertCategory(category)}</li>
      </Link>
      {showMore && (
        <ul
          onMouseEnter={() => setHasVisited(true)}
          onMouseLeave={() => setShowMore(false)}
          className="absolute bg-white border border-gray-300 p-2 rounded shadow-lg w-[90dvw] left-1/2 -translate-x-1/2"
        >
