...
export default async function Feedback({ user_id }: { user_id: number }) {
  ...
  return feedback ? (
    feedback.length === 0 ? (
      <section className="mobile-landscape:px-0 mobile-landscape:mx-0 my-4 grid grid-cols-12">
        <h2 className="col-span-full text-center text-sm">No Seller Rating for {user_id}</h2>
      </section>
    ) : (
      <section className="mobile-landscape:px-0 mobile-landscape:mx-0 my-4 grid grid-cols-12">
